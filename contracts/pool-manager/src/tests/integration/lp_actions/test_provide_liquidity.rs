...
fn test_provide_liquidity() {
    ...
    println!("Farm Manager Contract: Some(\"{}\")", suite.farm_manager_addr);
    println!("Fee Collector Contract: Some(\"{}\")", suite.fee_collector_addr);
    println!("Epoch Manager Contract: Some(\"{}\")", suite.epoch_manager_addr);
    ...
    println!("Looking for pool with assets: {} and {}", DENOM_UOM, UUSDY_DENOM);
    ...
    suite.instantiate_default().add_one_epoch().create_pool(
        ...
        |result| {
            ...
            assert!(!response.events.is_empty(), "Pool creation should generate events");
        },
    );
    ...
    suite.provide_liquidity(
        ...
        None,                    // liquidity_max_slippage - no slippage check for initial liquidity
        None,                    // swap_max_slippage - no slippage check for initial liquidity
        None,                    // receiver
        initial_assets,
        |result| {
            match result {
                Ok(tx_response) => {
                    ...
                    assert!(!tx_response.events.is_empty(), "Initial liquidity provision should generate events");
                }
                Err(e) => {
    ...
    println!("Assets: [");
    for asset in &assets {
        println!("    Coin {{ denom: \"{}\", amount: Uint128({}) }},", asset.denom, asset.amount);
    }
    println!("]");
        ...
        |result| {
            match result {
                Ok(tx_response) => {
                    ...
                    assert!(!tx_response.events.is_empty(), "Liquidity provision should generate events");
    ...
    suite.query_pools(Some(POOL_ID.to_string()), None, None, |result| {
        ...
        for asset in &assets {
            println!("  - {}: {} ({})", 
                asset.denom,
                asset.amount,
                if asset.denom == DENOM_UOM { "OM" } 
                else if asset.denom == UUSDY_DENOM { "USDY" } 
                else { "Unknown" }
            );
        }
        
        // Calculate total value in pool
        let om_amount = assets.iter()
            .find(|coin| coin.denom == DENOM_UOM)
            .map(|coin| coin.amount)
            .unwrap_or_default();
        let usdy_amount = assets.iter()
            .find(|coin| coin.denom == UUSDY_DENOM)
            .map(|coin| coin.amount)
            .unwrap_or_default();
            
        println!("Total OM in pool: {} ({})", om_amount, om_amount.u128() as f64 / 1_000_000.0);
        println!("Total USDY in pool: {} ({})", usdy_amount, usdy_amount.u128() as f64 / 1_000_000.0);
    });
    ...
    suite.query_lp_supply(POOL_ID.to_string(), |result| {
        ...
        println!("Total LP Token Supply: {} {}", lp_supply.amount, lp_supply.denom);
    });
    
    // Query user's LP token balance
    suite.query_balance(&creator.to_string(), lp_denom.clone(), |result| {
        let balance = result.unwrap();
        println!("User LP Token Balance: {} {}", balance.amount, balance.denom);
    });
    ...
    suite.query_all_balances(&creator.to_string(), |result| {
        ...
        for balance in &balances {
            if balance.denom == DENOM_UOM {
                println!("  - OM: {} ({})", balance.amount, balance.amount.u128() as f64 / 1_000_000.0);
            } else if balance.denom == UUSDY_DENOM {
                println!("  - USDY: {} ({})", balance.amount, balance.amount.u128() as f64 / 1_000_000.0);
