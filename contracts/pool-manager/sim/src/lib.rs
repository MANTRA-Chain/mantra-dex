...
impl Model {
    ...
    pub fn new(amp_factor: u64, balances: Vec<u128>, n_coins: u8) -> Model {
        let src_file = File::open(FILE_PATH);
        let mut src_file = match src_file {
            Ok(file) => file,
            Err(error) => {
                panic!("{error:?}\n Please run `curl -L
            https://raw.githubusercontent.com/curvefi/curve-contract/master/tests/simulation.py > sim/simulation.py`")
            }
        };
        let mut src_content = String::new();
        let _ = src_file.read_to_string(&mut src_content);

        assert_eq!(balances.len(), n_coins.into());
        ...
        Self {
            ...
            balances,
            n_coins,
            target_prices: vec![DEFAULT_TARGET_PRICE; n_coins.into()],
