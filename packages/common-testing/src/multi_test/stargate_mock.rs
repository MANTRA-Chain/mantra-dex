...
use cosmwasm_std::{
    coin, coins, to_json_binary, Addr, AnyMsg, Api, BankMsg, Binary, BlockInfo, CustomMsg,
    CustomQuery, MsgResponse, Querier, Storage, SubMsgResponse,
};
...
impl Stargate for StargateMock {
    ...
    {
        handle_stargate_any(api, storage, router, block, sender, msg.type_url, msg.value)
    }
    ...
    where
        ...
        QueryC: CustomQuery + DeserializeOwned + 'static,
    {
        handle_stargate_any(api, storage, router, block, sender, type_url, value)
    }
